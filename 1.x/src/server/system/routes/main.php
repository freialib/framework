 ...
 * ...
 */
function router($domain, $syspath, \fenrir\HttpDispatcher $http, \hlin\archetype\Context $context, \fenrir\system\MysqlDatabaseSignature $mysql) {
	...
	if ($http->matches('/api/.*')) {
		...
		try {
			...
			register_shutdown_function(function () {
				...
				if ($death !== null && in_array($death['type'], [E_ERROR, E_CORE_ERROR, E_COMPILE_ERROR])) {
			...
			\example\routes\api_v1_router($domain, $syspath, $routes, $http, $mysql);
		...
		try {
			...
			\example\routes\webapp_router($domain, $syspath, $routes, $http, $mysql);
